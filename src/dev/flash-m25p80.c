...
#define DB_PRINT_L(fmt, ...)                                                  \
  do {                                                                        \
    if (M25P80_ERR_DEBUG) { eprintf(": %s: " fmt, __func__, ##__VA_ARGS__); } \
  } while (0);
...
#define INFO(_part_name, _jedec_id, _ext_id, _sector_size, _n_sectors, _flags) \
  ...
  .sector_size = (_sector_size), .n_sectors = (_n_sectors), .page_size = 256,  \
  .flags = (_flags), .die_cnt = 0

#define INFO6(                                                           \
    _part_name, _jedec_id, _ext_id, _sector_size, _n_sectors, _flags)    \
  .part_name = _part_name,                                               \
  .id =                                                                  \
      {                                                                  \
          ((_jedec_id) >> 16) & 0xff,                                    \
  ...
  .id_len = 6, .sector_size = (_sector_size), .n_sectors = (_n_sectors), \
  .page_size = 256, .flags = (_flags), .die_cnt = 0
...
static inline Manufacturer get_man(Flash *s) {
  switch (s->pi->id[0]) {
  case 0x20: return MAN_NUMONYX;
  case 0xEF: return MAN_WINBOND;
  case 0x01: return MAN_SPANSION;
  case 0xC2: return MAN_MACRONIX;
  case 0xBF: return MAN_SST;
  default: return MAN_GENERIC;
  }
}

static inline uint32_t deposit32(
    uint32_t value, int start, int length, uint32_t fieldval) {
  uint32_t mask;
...
static void flash_erase(Flash *s, int offset, FlashCMD cmd) {
  ...
  case ERASE4_32K:
    ...
    break;
  case ERASE_SECTOR:
  case ERASE4_SECTOR: len = s->pi->sector_size; break;
  case BULK_ERASE: len = s->size; break;
  case DIE_ERASE:
    if (s->pi->die_cnt) {
      len = s->size / s->pi->die_cnt;
      offset = offset & (~(len - 1));
    } else {
      DB_PRINT_L("M25P80: die erase is not supported by device\n");
      return;
    }
    break;
  default: abort();
  ...
  if (!s->write_enable) {
    DB_PRINT_L("M25P80: erase with write protect!\n");
    return;
  }
...
static inline void flash_write8(Flash *s, uint32_t addr, uint8_t data) {
  uint32_t page = addr / s->pi->page_size;
  uint8_t prev = s->storage[s->cur_addr];

  if (!s->write_enable) { DB_PRINT_L("M25P80: write with write protect!\n"); }

  if ((prev ^ data) & data) {
    DB_PRINT_L("programming zero to one! addr=%" PRIx32 "  %" PRIx8
               " -> %" PRIx8 "\n",
        addr, prev, data);
  }
...
static inline int get_addr_length(Flash *s) {
  ...
  case QOR4:
  case DIOR4: return 4;
  default: return s->four_bytes_address_mode ? 4 : 3;
  }
}
...
static void complete_collecting_data(Flash *s) {
  ...
  case PP:
  case PP4:
  case PP4_4: s->state = STATE_PAGE_PROGRAM; break;
  case READ:
  case READ4:
  case FAST_READ:
  case FAST_READ4:
  ...
  case DIOR4:
  case QIOR:
  case QIOR4: s->state = STATE_READ; break;
  case ERASE_4K:
  case ERASE4_4K:
  case ERASE_32K:
  case ERASE4_32K:
  case ERASE_SECTOR:
  case ERASE4_SECTOR:
  case DIE_ERASE: flash_erase(s, s->cur_addr, s->cmd_in_progress); break;
  case WRSR:
    switch (get_man(s)) {
    case MAN_SPANSION: s->quad_enable = !!(s->data[1] & 0x02); break;
    case MAN_MACRONIX:
    ...
    if (s->write_enable) { s->write_enable = false; }
    break;
  case BRWR:
  case EXTEND_ADDR_WRITE: s->ear = s->data[0]; break;
  case WNVCR: s->nonvolatile_cfg = s->data[0] | (s->data[1] << 8); break;
  case WVCR: s->volatile_cfg = s->data[0]; break;
  case WEVCR: s->enh_volatile_cfg = s->data[0]; break;
  case RDID_90:
  case RDID_AB:
    if (get_man(s) == MAN_SST) {
      if (s->cur_addr <= 1) {
        ...
        DB_PRINT_L("M25P80: Invalid read id address\n");
      }
    } else {
      DB_PRINT_L(
...
static void reset_memory(Flash *s) {
  ...
  case MAN_NUMONYX:
    ...
    if ((s->nonvolatile_cfg & NVCFG_XIP_MODE_MASK) != NVCFG_XIP_MODE_DISABLED) {
      s->volatile_cfg |= VCFG_XIP_MODE_ENABLED;
    }
    s->volatile_cfg |= deposit32(s->volatile_cfg, VCFG_DUMMY_CLK_POS,
        CFG_DUMMY_CLK_LEN,
        extract32(s->nonvolatile_cfg, NVCFG_DUMMY_CLK_POS, CFG_DUMMY_CLK_LEN));
    ...
    break;
  case MAN_MACRONIX: s->volatile_cfg = 0x7; break;
  case MAN_SPANSION:
    ...
    s->quad_enable = extract32(
        s->spansion_cr1v, SPANSION_QUAD_CFG_POS, SPANSION_QUAD_CFG_LEN);
    s->four_bytes_address_mode = extract32(
        s->spansion_cr2v, SPANSION_ADDR_LEN_POS, SPANSION_ADDR_LEN_LEN);
    break;
  default: break;
  }

  DB_PRINT_L("Reset done.\n");
}
...
static void decode_fast_read_cmd(Flash *s) {
  ...
  case MAN_WINBOND: s->needed_bytes += 8; break;
  case MAN_NUMONYX: s->needed_bytes += extract32(s->volatile_cfg, 4, 4); break;
  case MAN_MACRONIX:
    ...
    break;
  case MAN_SPANSION:
    s->needed_bytes += extract32(
        s->spansion_cr2v, SPANSION_DUMMY_CLK_POS, SPANSION_DUMMY_CLK_LEN);
    break;
  default: break;
  }
  s->pos = 0;
...
static void decode_dio_read_cmd(Flash *s) {
  ...
  case MAN_MACRONIX:
    ...
    case 1: s->needed_bytes += 6; break;
    case 2: s->needed_bytes += 8; break;
    default: s->needed_bytes += 4; break;
    }
    break;
  default: break;
  }
...
static void decode_qio_read_cmd(Flash *s) {
  ...
  case MAN_MACRONIX:
    switch (extract32(s->volatile_cfg, 6, 2)) {
    case 1: s->needed_bytes += 4; break;
    case 2: s->needed_bytes += 8; break;
    default: s->needed_bytes += 6; break;
    }
    break;
  default: break;
  }
...
static void decode_new_cmd(Flash *s, uint32_t value) {
  ...
  int i;
  DB_PRINT_L("decoded new command:%x\n", value);

  if (value != RESET_MEMORY) { s->reset_enable = false; }
  ...
  case DOR4:
  case QOR:
  case QOR4: decode_fast_read_cmd(s); break;

  case DIOR:
  case DIOR4: decode_dio_read_cmd(s); break;

  case QIOR:
  case QIOR4: decode_qio_read_cmd(s); break;
  ...
  case WRSR:
    if (s->write_enable) {
      ...
      case MAN_MACRONIX:
        ...
        break;
      default: s->needed_bytes = 1; s->state = STATE_COLLECTING_DATA;
      }
      s->pos = 0;
    }
    break;

  case WRDI: s->write_enable = false; break;
  case WREN: s->write_enable = true; break;

  case RDSR:
    s->data[0] = (!!s->write_enable) << 1;
    if (get_man(s) == MAN_MACRONIX) { s->data[0] |= (!!s->quad_enable) << 6; }
    s->pos = 0;
    s->len = 1;
    s->data_read_loop = true;
    s->state = STATE_READING_DATA;
  ...
  case JEDEC_READ:
    DB_PRINT_L("populated jedec code\n");
    for (i = 0; i < s->pi->id_len; i++) { s->data[i] = s->pi->id[i]; }
  ...
  case BULK_ERASE:
    ...
    break;
  case NOP: break;
  case EN_4BYTE_ADDR: s->four_bytes_address_mode = true; break;
  case EX_4BYTE_ADDR: s->four_bytes_address_mode = false; break;
  case BRRD:
  ...
  case WEVCR:
    ...
    break;
  case RESET_ENABLE: s->reset_enable = true; break;
  case RESET_MEMORY:
    if (s->reset_enable) { reset_memory(s); }
    break;
  case RDCR_EQIO:
    switch (get_man(s)) {
    case MAN_SPANSION:
      ...
      s->state = STATE_READING_DATA;
      break;
    case MAN_MACRONIX: s->quad_enable = true; break;
    default: break;
    }
    break;
  case RSTQIO: s->quad_enable = false; break;
  default: DB_PRINT_L("M25P80: Unknown cmd %x\n", value); break;
  }
}

static int m25p80_cs(Flash *s, bool select) {
  if (select) {
...
static uint32_t m25p80_transfer8(Flash *s, uint32_t tx) {
  uint32_t r = 0;

  switch (s->state) {

  case STATE_PAGE_PROGRAM:
    DB_PRINT_L("page program cur_addr=%#" PRIx32 " data=%" PRIx8 "\n",
  ...
  case STATE_READ:
    ...
    DB_PRINT_L("READ 0x%" PRIx32 "=%" PRIx8 "\n", s->cur_addr, (uint8_t)r);
    s->cur_addr = (s->cur_addr + 1) & (s->size - 1);
    break;
  ...
  case STATE_COLLECTING_VAR_LEN_DATA:
    ...
    if (s->len == s->needed_bytes) { complete_collecting_data(s); }
    break;
  ...
  default:
  case STATE_IDLE: decode_new_cmd(s, (uint8_t)tx); break;
  }

  return r;
}

static void m25p80_init(Flash *s) {
  /* FIXME:
...
// static void m25p80_reset(Flash *s) { reset_memory(s); }
